...
export default function ModuleLesson({ module, onBack, onProgress, durationMinutes }) {
  ...
  useEffect(() => {
    if (secondsLeft === null || finished || secondsLeft === 0) return
    const id = setTimeout(() => {
      if (secondsLeft === 1) {
        setSecondsLeft(0)
        setFinished(true)
  ...
  if (finished) {
    ...
    return (
      <section className="card session-summary">
        <span className="eyebrow dark">Session finished! 🎉</span><h2>High five, {module.title.toLowerCase()} star!</h2>
        <p className="summary-intro">You played a short set of questions. Check your star chart and choose the next mini mission to explore.</p>
        ...
        <div className="summary-actions"><button className="primary-action" onClick={restart}>Try this mission again</button><button className="secondary-action bordered" onClick={onBack}>Back to mini missions</button></div>
      </section>
    )
  }
  ...
  return (
    <section className="card module-lesson">
      <div className="hud"><button className="back-btn" onClick={onBack}>← Categories</button>{secondsLeft !== null && <span className={`timer-chip${secondsLeft <= 60 ? ' low' : ''}`} role="timer" aria-label="Time remaining">Time {String(Math.floor(secondsLeft / 60)).padStart(2, '0')}:{String(secondsLeft % 60).padStart(2, '0')}</span>}<span className="lesson-position"><span>{module.bm}</span><strong>{stepIndex + 1} of {module.steps.length}</strong></span></div>
      <div className="progress-track"><div className="progress-fill" style={{ width: `${((stepIndex + 1) / module.steps.length) * 100}%` }} /></div>
      <span className="eyebrow dark">{module.title} · Let’s play!</span>
      {step.points != null && <span className="points-badge">{step.points} points</span>}<h2 className="question">{step.prompt}</h2>
      ...
      <div id="feedback" className={selected === null ? '' : correct ? 'good' : 'bad'}>{selected === null ? 'Take a look, then pick your best answer!' : correct ? `🌟 ${step.explain}` : `Not yet — ${step.hint}`}</div>
      {correct && <button className="next-btn" onClick={next}>{stepIndex === module.steps.length - 1 ? 'See my star chart' : 'Next mini mission →'}</button>}
    </section>
  )
}
